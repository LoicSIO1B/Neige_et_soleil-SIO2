		...
		if($nomlength <= 255)
		{
			...
			if($prenomlength <=255 || $prenom != text)
			{
				if($mail1 == $mail2)
				{
					...
					{
						$reqmail =$bdd->prepare("SELECT * FROM proprietaire where mailPro = ?");
						...
						if($mailexist == 0)
						{
							...
							if($telephonelength = 10)
							{
								if($mdp1 == $mdp2)
								{
									$insertmbr= $bdd->prepare("INSERT INTO proprietaire(nomPro, prenomPro, mailPro, telephonePro, motdepassePro) VALUES(?,?,?,?,?)");
		...
		<section class="ombre">
			...
			<section class="contenu ombre">
				<section class="contenu ombre">
					<div id="tabs">
						<ul>
						     <li><a href="inscription.php" onclick="loadit(this)" >Client</a></li>
						     <li><a href="inscriptionProprio.php" onclick="loadit(this)" class="selected">Propriétaire</a></li>
						</ul>
					</div>
