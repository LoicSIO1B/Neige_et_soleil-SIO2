...
//récupération des listes logement ******************************************************************
  ...
  //test des logements à traiter *************************************************************************
  if ( ($logement == 0 || $logement == $cle) && $cle <> 0  ) {
  ...
  foreach ($tableau_reservation[$cle] as $date_index => $val_reservation ) {
    ...
    //si locataire différent de id à supprimer alors on ajout la date ******
      if( comparaison_date ($date_temp,$date_index,"superieur","-","eng") || ( $locataire <> 0 && ( $locataire == 0 || $locataire <> $tri_locataire_temp )) ) {
