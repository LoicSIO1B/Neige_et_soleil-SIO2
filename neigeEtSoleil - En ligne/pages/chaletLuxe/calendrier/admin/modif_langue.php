...
if ( isset($_POST['Modifier']) && ($_POST['Modifier']) == 'Modifier' && !(empty($_POST['symbole_langue']))  && !MODE_DEMO ) {
...
$cle_langue_modif = $num_supprime;
$nom_langue_temp = $nom_langue[$num_supprime] ;
$symbole_langue_temp = $num_supprime ;
$lundi_temp = $lundi[$num_supprime];
$mardi_temp = $mardi[$num_supprime];
$mercredi_temp = $mercredi[$num_supprime];
$jeudi_temp = $jeudi[$num_supprime];
$vendredi_temp = $vendredi[$num_supprime];
$samedi_temp = $samedi[$num_supprime];
$dimanche_temp = $dimanche[$num_supprime];
$semaine_temp = $semaine[$num_supprime];
$janvier_temp = $janvier[$num_supprime];
$fevrier_temp = $fevrier[$num_supprime];
$mars_temp = $mars[$num_supprime];
$avril_temp = $avril[$num_supprime];
$mai_temp = $mai[$num_supprime];
$juin_temp = $juin[$num_supprime];
$juillet_temp = $juillet[$num_supprime];
$aout_temp = $aout[$num_supprime];
$septembre_temp = $septembre[$num_supprime];
$octobre_temp = $octobre[$num_supprime];
$novembre_temp = $novembre[$num_supprime];
$decembre_temp = $decembre[$num_supprime];
$periode_temp = $periode[$num_supprime];
$du_temp = $du[$num_supprime];
$au_temp = $au[$num_supprime];
$maj_temp = $maj[$num_supprime];
$semaine_periode_temp = $semaine_periode[$num_supprime];

if ( $cle_langue_modif == 'fr' )
    $verrou = 'readonly' ;
 }
...
<link href="modif_langue.css" rel="stylesheet" type="text/css">
...
function ValidateForm1(theForm)
{
   ...
   if (theForm.maj_post.value == "")
   {
      alert("Vous n'avez pas renseigné le texte pour septembre!");
      theForm.maj_post.focus();
      return false;
   }
   if (theForm.maj_post.value.length < 1)
   {
      alert("Vous n'avez pas renseigné le texte pour septembre!");
      theForm.maj_post.focus();
      return false;
...
<div id="space"><br></div>
<div id="container">
<table style="position:absolute;left:5px;top:5px;width:906px;height:438px;z-index:57;" cellpadding="5" cellspacing="1" id="Table1">
<tr>
...
<img src="images/langue_modif.png" id="Image2" alt=""></div>
<div id="Html2" style="position:absolute;left:87px;top:12px;width:120px;height:23px;z-index:59">
<?php
...
?></div>
<div id="wb_Form2" style="position:absolute;left:13px;top:41px;width:886px;height:387px;z-index:60;">
<form name="Form1" method="post" action="modif_langue.php" id="Form2" onsubmit="return ValidateForm1(this)">
<input type="hidden" name="cle_modif" value="<?php echo $cle_langue_modif; ?>">
...
<input type="submit" id="Button1" name="Modifier" value="Modifier" style="position:absolute;left:17px;top:344px;width:96px;height:25px;z-index:2;">
...
<input type="button" id="Button2" onclick="parent.parent.location = 'edit_langue.php' ;return false;" name="" value="Fermer" style="position:absolute;left:755px;top:344px;width:96px;height:25px;z-index:54;" tabindex="6">
<input type="text" id="maj_post" style="position:absolute;left:19px;top:307px;width:165px;height:18px;line-height:18px;z-index:55;" name="maj_post" value="<?php echo html_ent($maj_temp); ?>">
<div id="wb_Text28" style="position:absolute;left:20px;top:278px;width:166px;height:19px;text-align:center;z-index:56;">
